...
import { Anchor, Loader2, Pencil, Plus, Trash2, Ship, Info, Calendar } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">{isHr ? "Kopnena mjesta (Suhi vez)" : "Dry Berths"}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {zones.filter(z => z.isActive).map((zone) => {
            ...
            return (
              <Card 
                key={zone.id} 
                className={`relative overflow-hidden cursor-pointer border transition-all hover:shadow-md rounded-2xl ${
            ...
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-lg font-bold">
                  {isHr ? `Aktivna plovila — ${activeZone.name}` : `Active Vessels — ${activeZone.name}`}
                </CardTitle>
                <CardDescription>
                  {isHr ? "Popis plovila koja se trenutno nalaze na suhom vezu u ovoj zoni." : "List of vessels currently stored on land in this zone."}
                </CardDescription>
              </div>
              <Badge className="bg-primary/10 border-primary/20 text-primary font-semibold py-1 px-3">
                {isHr ? `Ukupno: ${occupancies.length}` : `Total: ${occupancies.length}`}
              </Badge>
            </div>
          </CardHeader>
